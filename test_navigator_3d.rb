...
class TestGameOfLife < Test::Unit::TestCase
  ...
  def setup
    ...
    @nav = XYZ_Surveyor.new
  end

  def test_navigator_confirms_location_exists
    result = nav.living?({ x: 0, y: 0, z: 0 }, locations)
    assert_equal(true, result)
  end

  def test_navigator_counts_existing_neighbors_of_given_location
    result = nav.neighbors({ x: 0, y: 0, z: 0 }, locations)
    assert_equal(2, result)
    result = nav.neighbors({ x: -1, y: 0, z: 0 }, locations)
    assert_equal(2, result)
  end

  def test_navigator_informs_possible_influence_of_given_location
    result = nav.influence({ x: 0, y: 0, z: 0 }, locations)
    expected = [
      { x: -1, y: -1, z: -1 },
      { x: -1, y: -1, z:  0 },
      { x: -1, y: -1, z:  1 },
